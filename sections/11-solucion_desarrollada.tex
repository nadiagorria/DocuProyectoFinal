...
A continuación se presentan capturas de pantalla del sistema que ilustran la carga de lotes (Figuras \ref{fig:registro-lote} y \ref{fig:listado-lotes}), el flujo de trabajo de los análisis (Figura \ref{fig:registro-analisis}), los reportes generados (Figura \ref{fig:centro-reportes}) y los paneles de métricas (Figuras \ref{fig:panel-metricas-1} y \ref{fig:panel-metricas-2}).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{registroDeLote.png}
    \caption{Pantalla de registro de lotes permitiendo crear y editar la ficha de cada lote, asociar cultivares y empresas, y consultar los últimos lotes ingresados.}
...
\begin{figure}[htbp]
    ...
    \caption{Pantalla de listado de lotes con búsqueda, filtros y acceso al detalle de cada lote y sus análisis asociados.}
...
\begin{figure}[htbp]
    ...
    \caption{Pantalla de registro de análisis con selección del tipo de análisis (Pureza Física, Germinación, DOSN, PMS, Tetrazolio) y asociación al lote correspondiente.}
...
\begin{figure}[htbp]
    ...
    \caption{Centro de reportes con generación y exportación de reportes a Excel con filtros configurables.}
...
\begin{figure}[htbp]
    ...
    \caption{Panel de métricas mostrando indicadores de análisis por período.}
...
\begin{figure}[htbp]
    ...
    \caption{Panel de métricas mostrando distribución por tipo y estado.}
